...
class TestCreateAgent:
    ...
    def test_planner_uses_separate_backend_when_configured(self):
        ...
        a distinct backend from the one ReActLoop uses -- the whole point
        of create_planner_backend()."""
        from chef_human.config import Settings

        planner_settings = Settings(
            llm_backend="ollama", planner_ollama_model="small-model"
        )
        with (
            patch("chef_human.config.settings", planner_settings),
            patch("chef_human.llm.create_backend") as mock_create_backend,
            patch("chef_human.agent.create_context_assembler") as mock_ctx_factory,
            patch("chef_human.tools.create_tool_registry"),
            patch("chef_human.agent.planner.Planner") as mock_planner_cls,
            patch("chef_human.agent.react_loop.ReActLoop") as mock_loop_factory,
            patch("chef_human.ui.protocol.NoopUI"),
        ):
            from chef_human.agent import create_agent

            main_backend = MagicMock(name="main_backend")
            planner_backend = MagicMock(name="planner_backend")
            mock_create_backend.side_effect = [main_backend, planner_backend]
            ...
            mock_planner_cls.assert_called_once_with(llm_backend=planner_backend)
            _, loop_kwargs = mock_loop_factory.call_args
            assert loop_kwargs["llm_backend"] is main_backend

    def test_planner_reuses_main_backend_by_default(self):
        ...
        with (
            patch("chef_human.config.settings", default_settings),
            patch("chef_human.llm.create_backend") as mock_create_backend,
            patch("chef_human.agent.create_context_assembler") as mock_ctx_factory,
            patch("chef_human.tools.create_tool_registry"),
            patch("chef_human.agent.planner.Planner") as mock_planner_cls,
            ...
            main_backend = MagicMock(name="main_backend")
            mock_create_backend.return_value = main_backend
            ...
            mock_planner_cls.assert_called_once_with(llm_backend=main_backend)
            _, loop_kwargs = mock_loop_factory.call_args
            assert loop_kwargs["llm_backend"] is main_backend
