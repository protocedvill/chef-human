...
class TestTuiUIProtocol:
    ...
    async def test_on_ask_user_submits_typed_answer(self, tmp_path):
        ...
        through Textual's own event loop -- by taking over the #task-input
        bar inline, not a modal popup (obtrusive, and prone to clipping
        long questions in a fixed-width dialog)."""
        app, ui = await self._ui(tmp_path)
        async with app.run_test() as pilot:
            results: list[str] = []
            ...
            app.run_worker(request())
            await pilot.pause()
            assert "hidden" not in app.query_one("#question-banner").classes
            await pilot.click("#task-input")
            for ch in "SQLite":
                await pilot.press(ch)
            await pilot.press("enter")
            ...
            assert "hidden" in app.query_one("#question-banner").classes
    ...
    async def test_on_ask_user_skip_returns_default_message(self, tmp_path):
        ...
        async with app.run_test() as pilot:
            ...
            app.run_worker(request())
            await pilot.pause()
            await pilot.click("#skip-question-btn")
            await pilot.pause()
            assert results == ["User skipped the question"]

    async def test_ask_user_does_not_leak_into_next_task_submission(self, tmp_path):
        """Regression test: answering via Enter must not also submit the
        same text as a brand-new top-level task -- #task-input is shared
        between task-submission mode and answer-collection mode, routed by
        whether a question is pending."""
        submitted_tasks: list[str] = []

        async def on_submit(text: str) -> None:
            submitted_tasks.append(text)
        ...
        async with app.run_test() as pilot:
            ...
            async def request() -> None:
                await ui.on_ask_user("Which database should I use?")
...
class TestStatsPanelUpdates:
    ...
    async def test_on_tool_result_error_adds_warning(self, tmp_path):
        ...
        async with app.run_test():
            ...
            assert "Warnings: 1" in self._panel_text(app)
    ...
    async def test_on_plan_progress_updates_current_step_live(self, tmp_path):
        """current_step/steps_done must refresh every turn as steps
        complete, not just once at on_plan() (task start) and once more at
        display_result() (task end) -- otherwise the panel's "Step:" line
        ...
        async with app.run_test():
            ...
            ui.on_plan_progress(plan)
            assert ui.stats.current_step == "step two"
            assert ui.stats.steps_done == 1
            text = self._panel_text(app)
    ...
    async def test_no_warnings_message_when_clean(self, tmp_path):
        ...
        async with app.run_test():
            ui.render_stats()
            assert "Warnings: 0" in self._panel_text(app)


class TestTreePaneRefresh:
    """Regression tests: #tree-pane's DirectoryTree is only populated once
    at compose() time and never re-scans disk on its own, so it goes stale
    the moment the agent creates/edits/deletes a file. on_tool_result must
    reload it after any file-mutating tool succeeds."""

    async def _ui(self, tmp_path):
        app, _ = await _make_app(tmp_path)
        return app, app.tui_ui

    async def test_reloads_after_successful_write(self, tmp_path):
        from unittest.mock import patch
        app, ui = await self._ui(tmp_path)
        async with app.run_test():
            with patch.object(app.query_one("#tree-pane"), "reload") as mock_reload:
                ui.on_tool_result("write", "Wrote 3 lines to foo.py")
                mock_reload.assert_called_once()

    async def test_does_not_reload_after_failed_write(self, tmp_path):
        from unittest.mock import patch
        app, ui = await self._ui(tmp_path)
        async with app.run_test():
            with patch.object(app.query_one("#tree-pane"), "reload") as mock_reload:
                ui.on_tool_result("write", "Error: Outside workspace: /etc/hosts")
                mock_reload.assert_not_called()

    async def test_does_not_reload_for_non_mutating_tool(self, tmp_path):
        from unittest.mock import patch
        app, ui = await self._ui(tmp_path)
        async with app.run_test():
            with patch.object(app.query_one("#tree-pane"), "reload") as mock_reload:
                ui.on_tool_result("read", "print('hi')")
                mock_reload.assert_not_called()
